...
namespace octotree {
    ...
    public:
        ...
        octonode_t(geometry::cube_t cube, ListT* parent_figs, const octotree_t& tree) : cube_(cube), parent_figs_(parent_figs), tree_(tree)
        {
            figs_ = new ListT;
        }
        ~octonode_t()
        ...
        void fill_figures_in_node()
        {
            ...
            for (ListIterT list_it = parent_figs_->begin(); list_it != parent_figs_->end(); list_it++)
            {
        ...
        void intersect_node_with_node(octonode_t &other, std::set<size_t> &intersect_figs_id)
        {
        ...
        void intersect_with_subtree(octonode_t &subtree, std::set<size_t> &intersect_figs_id)
        {
            ...
            for (size_t child_num = 0; child_num < NUM_CHILDREN; child_num++)
                intersect_with_subtree(*(subtree.children_[child_num]), intersect_figs_id);
        }
    ...
    void octonode_t::fill_octonode()
    {
        ...
        if (real_nums::is_less_or_equal_zero(cube_.x2_ - cube_.x1_ - tree_.cube_min_size_) || figs_->size() < 3)
        {
            for (size_t child_num = 0; child_num < NUM_CHILDREN; child_num++)
