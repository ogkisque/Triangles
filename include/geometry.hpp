...
bool is_point_on_line(const point_t &point, const line_t &line,
                      bool is_on_segment);
...
bool is_line_intersect_triangle_2d(const line_t &line,
                                   const triangle_t &triangle);
bool is_line_intersect_triangle(const line_t &line, const triangle_t &triangle);
bool is_triangle_intersect_triangle(const triangle_t &triangle1,
...
get_line_plane_intersection(const line_t &line, const plane_t &plane);
...
bool is_line_intersect_line(const line_t &line1, const line_t &line2) {
    ...
    if (vec_multiply.is_null()) // parallel or identic
    {
        if (is_point_on_line(line1.p1_, line2, false)) // identic lines
            return is_point_on_line(line1.p1_, line2, true) ||
                   is_point_on_line(line1.p2_, line2, true);
...
bool is_line_intersect_triangle_2d(const line_t &line,
                                   const triangle_t &triangle) {
    if (is_point_in_triangle(line.p1_, triangle) ||
        is_point_in_triangle(line.p2_, triangle))
        return true;
...
bool is_line_intersect_triangle(const line_t &line,
                                const triangle_t &triangle) {
...
get_line_plane_intersection(const line_t &line, const plane_t &plane) {
    ...
    double plane_equation = line.p1_.x_ * plane.a_ + line.p1_.y_ * plane.b_ +
                            line.p1_.z_ * plane.c_ + plane.d_;
